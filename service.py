...
add_supported_tools(
    {
        ...
        "urn:sd-core:crewai.builtin.directoryReadTool": lambda _, ctxt: (
            DirectoryReadTool(directory=ctxt.inputs_dir)
            if ctxt.inputs_dir
            else DirectoryReadTool(directory=".")
        ),
        # DirectorySearchTool - requires inputs_dir (inherits embedder from Crew)
        "urn:sd-core:crewai.builtin.directorySearchTool": lambda _, ctxt: DirectorySearchTool(
...
def get_auth_token(job_ctxt: JobContext) -> Optional[str]:
    ...
    if hasattr(job_ctxt, "job_authorization"):
        ...
        if token:
            ...
            if isinstance(token, str) and token.startswith("Bearer "):
                logger.info(
                    f"✓ JWT extracted from job_authorization (with Bearer prefix, length: {len(token)-7})"
                )
                return token[7:]
            logger.info(
                f"✓ JWT extracted from job_authorization (length: {len(str(token))})"
            )
            return token
    ...
    logger.warning(
        "✗ No JWT token found in any path (job_authorization, auth_token, headers, request.headers)"
    )
    return None
    ...
    try:
        ...
        if req.context_urns:
            ...
            if inputs_dir:
                ...
                if pdf_files:
                    logger.info(
                        f"📄 Detected {len(pdf_files)} PDF file(s) - PDFSearchTool recommended"
                    )
                    logger.info(
                        f"   Files: {', '.join([f.name for f in pdf_files[:5]])}"
                    )
                if text_files:
                    logger.info(
                        f"📝 Detected {len(text_files)} text file(s) - DirectorySearchTool recommended"
                    )
                    logger.info(
                        f"   Files: {', '.join([f.name for f in text_files[:5]])}"
                    )
                if not pdf_files and not text_files:
                    logger.warning(
                        "⚠ No recognized file types (PDF/TXT/MD/CSV) - DirectoryReadTool can list files"
                    )
            else:
        ...
        if req.context_urns and inputs_dir:
            ...
            if use_tools:
                logger.info(
                    "🔧 Crew has DirectoryReadTool or research/search agents - using tool injection"
                )
                ...
                for agent in crew_def.agents:
                    ...
                    if has_directory_read or has_research_goal:
                        ...
                        logger.info(
                            f"  Agent '{agent.name}' qualified for tool injection (has_directory_read={has_directory_read}, has_research_goal={has_research_goal})"
                        )

                if not agents_needing_tools:
                    logger.warning(
                        "⚠ Tool mode detected but no agents qualified for tool injection"
                    )
                ...
                for agent in agents_needing_tools:
                    ...
                    if not has_directory_read:
                        ...
                        logger.info(
                            f"  → Auto-injected DirectoryReadTool into agent '{agent.name}'"
                        )
                    ...
                    if pdf_files:
                        ...
                        if not has_pdf_search:
                            ...
                            logger.info(
                                f"  → Auto-injected PDFSearchTool into agent '{agent.name}'"
                            )
                ...
                try:
                    ...
                    logger.info(
                        "  All agents will have automatic RAG access to artifacts"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to create artifact knowledge sources: {e}",
                        exc_info=True,
                    )
        ...
        try:
            planning_test_response = planning_llm.call(
        ...
        if knowledge_sources:
            ...
            if embedder_config:
                ...
                logger.warning(
                    "  ⚠ Knowledge sources without embedder - may use default OpenAI"
                )
        ...
        logger.info(
            f"✓ Crew built: {len(crew.agents)} agents, " f"{len(crew.tasks)} tasks"
        )
        ...
        if embedder_config:
            ...
            logger.info(
                f"✓ RAG tools enabled: DirectorySearchTool will index files in {inputs_dir}"
            )
        ...
        try:
            with open(final_output_file, "w", encoding="utf-8") as f:
                f.write(f"# {req.name}\n\n")
                f.write(
                    f"**Executed:** {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                )
